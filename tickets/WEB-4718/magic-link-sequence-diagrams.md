...
**Note:** The Token Redemption step shows both deprecated and current API endpoints, similar to Flow 1.
...
== Token Redemption (API MIGRATION) ==
User -> Browser : Click magic link
Browser -> InviteComp : /org-user-invite/:token
InviteComp -> Controller : redeemMagicLinkToken(institutionId, token)

group #Pink DEPRECATED: Legacy Endpoint
    Controller -> API : PUT /v1/institutions/{id}/token/{token}/redeem
    API --> Controller : 200 OK + {userId, redirectUri: "2fa-landing-page/OrgName"}
end

group #LightGreen CURRENT: New Endpoint
    Controller -> Controller : decodeMagicLinkToken(token)
    note right of Controller : Extract userId from JWT
    Controller -> API : PUT /v1/institutions/{id}/users/{userId}/redeem?token=...
    API --> Controller : 200 OK + {redirectUri: "2fa-landing-page/OrgName"}
    Controller -> Controller : Merge userId into response
end

Controller --> InviteComp : {userId, redirectUri}
...
**Note:** Scenario A shows both deprecated and current error paths. Scenario D is a NEW error scenario introduced with the API migration.
...
== Scenario A: Expired/Invalid Token (API MIGRATION) ==
User -> Component : Click old magic link
Component -> Controller : redeemMagicLinkToken(token)

group #Pink DEPRECATED: Legacy Error Path
    Controller -> API : PUT .../token/{token}/redeem
    API --> Controller : 401 Unauthorized
    Controller --> Component : REDEEM_UNAUTHORIZED event
end

group #LightGreen CURRENT: New Error Path
    Controller -> Controller : decodeMagicLinkToken(token)
    note right of Controller : Token decodes successfully
    Controller -> API : PUT .../users/{userId}/redeem?token=...
    API --> Controller : 401 Unauthorized
    Controller --> Component : REDEEM_UNAUTHORIZED event
end
...
note over User,API #LightGreen : This error scenario is NEW in the current implementation
User -> Component : Click magic link with malformed token
Component -> Controller : redeemMagicLinkToken(token)
Controller -> Controller : decodeMagicLinkToken(token)
note right of Controller : Token is not valid JWT format\n(not 3 parts, or not base64)
Controller --> Component : REDEEM_ERROR event (400)
note right of Controller : No API call made - fails client-side
Component -> User : "Invitation link failed"\n"Request another invite"
...
> **API Migration Note:** This entire process is **NEW** in the current implementation. The legacy endpoint did not require client-side token decoding - the API returned the `userId` in the response. The new endpoint requires the client to decode the JWT token to extract `userId` before making the API call.

```plantuml
@startuml
!theme plain
title Token Decoding Process (NEW in Current Implementation)

start

partition #LightPink "LEGACY: No client-side decoding" {
    :Token passed directly to API;
    :API returns userId in response;
    note right: No validation needed client-side
}

partition #LightGreen "CURRENT: Client-side JWT decoding" {
    :Token Received;
...
| Aspect | DEPRECATED (Legacy) | CURRENT (New) |
|--------|---------------------|---------------|
| **Dependencies** | None for token decoding | Uses `decodeMagicLinkToken` utility |
| **Token handling** | Passes token directly to API | Decodes JWT to extract userId first |
| **New utility file** | N/A | `src/js/util/magic-link-token.js` |

```plantuml
@startuml
!theme plain
title MagicLinkController Structure (with API Migration Changes)

package "LEGACY Structure" #Pink {
    class MagicLinkController_Old {
        ...
        --
        <i>No token decoding dependency</i>
    }
}

package "CURRENT Structure" #LightGreen {
    class MagicLinkController {
        +institution : Institution
        +redeemMagicLinkToken(institutionId, token) : Promise
        +getPhoneNumbers() : Promise
    ...
    class decodeMagicLinkToken <<utility>> <<NEW>> {
        +decodeMagicLinkToken(token) : Object
        -decodeBase64Url(str) : Object
    }

    MagicLinkController ..> decodeMagicLinkToken : <<NEW>> uses
}
...
note bottom of decodeMagicLinkToken
...
| Aspect | DEPRECATED (Legacy) | CURRENT (New) |
|--------|---------------------|---------------|
| **Token utility** | None | `decodeMagicLinkToken` utility added |
| **Controllers using utility** | None | `MagicLinkController`, `UserManagementController` |
| **AccountRecoveryController** | Unchanged | Unchanged (uses different endpoints) |

```plantuml
@startuml
!theme plain
title Controllers Overview (with API Migration Changes)
...
package "NEW: Token Utility" #LightGreen {
...
MagicLinkController ..> decodeMagicLinkToken : <<NEW>> uses
UserManagementController ..> decodeMagicLinkToken : <<NEW>> uses

note right of AccountRecoveryController
    **Unchanged** - uses different
...
note bottom of decodeMagicLinkToken
    **Migration Change:**
    Both MagicLinkController and
    UserManagementController now
    depend on this new utility
end note
